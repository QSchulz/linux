			    ...
			    unsigned int irq)
{
	...
	/* FIXME: add timeout to invalidate values */
	/*
	 * The temperature sensor throws periodically an interrupt. It is
	 * possible to read the temperature from sysfs but the thermal framework
	 * does also regular polling. If a sysfs read and thermal framework's
	 * polling happen during the same period, one will timeout whereas there
	 * is a temerature value that has been gotten.
	 * Using cached value avoids ugly messages sent by the thermal framework
	 * to notify the user of a timeout while getting the temperature value.
	 */
...
static irqreturn_t sun4i_gpadc_temp_data_irq_handler(int irq, void *dev_id)
{
	...
	/* FIXME: spinlock when reading and writing temp_data */
	if (!regmap_read(info->regmap, SUN4I_GPADC_TEMP_DATA, &info->temp_data))
		/* FIXME: disable irq when not in buffer mdoe */
